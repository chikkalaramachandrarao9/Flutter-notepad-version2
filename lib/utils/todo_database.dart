...
class DatabaseHelper {
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute('CREATE TABLE $noteTable($colTitle TEXT)');
  }
...
//  Future<int> updateNote(ToDo note, ToDo newNote) async {
...
//  }
  ...
  Future<int> deleteNote(ToDo note) async {
    var db = await this.database;
    String t = note.title;
    int result =
        await db.rawDelete('DELETE FROM $noteTable WHERE $colTitle = \'$t\'');
    return result;
  }
  ...
  Future<List<ToDo>> getToDoList() async {
    var noteMapList = await getToDoMapList(); // Get 'Map List' from database
    int count =
    ...
    for (int i = 0; i < count; i++) {
      noteList.add(ToDo.fromMapToObject(noteMapList[i]));
    }
